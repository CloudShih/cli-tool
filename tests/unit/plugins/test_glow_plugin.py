...
def test_glow_model():
    ...
    try:
        ...
        if available:
            ...
            print(f"❌ Glow 工具不可用: {error_message}")
            print("💡 請安裝 Glow 工具: https://github.com/charmbracelet/glow")
        ...
        for url in test_urls:
            ...
            if is_valid:
                print(f"✅ URL 有效: {url} → {processed_url}")
            else:
                print(f"❌ URL 無效: {url} - {error_msg}")
        
        # 測試快取功能
        print("\n📋 測試快取功能...")
        cache_info = model.get_cache_info()
        print(f"✅ 快取信息: {cache_info}")
...
"""
            ...
            if success:
                ...
                print(f"❌ Markdown 渲染失敗: {error_msg}")
        
        return True
        
    except Exception as e:
        print(f"❌ GlowModel 測試失敗: {e}")
        return False
...
def test_glow_plugin():
    ...
    try:
        ...
        print(f"📋 工具可用性: {'✅ 可用' if tool_available else '❌ 不可用'}")
        ...
        print(f"📋 配置選項數量: {len(config_schema)}")
        
        # 測試支援的檔案類型
        file_types = plugin.get_supported_file_types()
        print(f"📋 支援檔案類型: {', '.join(file_types)}")
        
        return True
        
    except Exception as e:
        print(f"❌ GlowPlugin 測試失敗: {e}")
        return False
...
def test_error_handling():
    ...
    try:
        ...
        # 空內容應該不會導致崩潰，但可能會有警告
        print(f"📄 空內容渲染結果: {'成功' if success else f'失敗 - {error_msg}'}")
